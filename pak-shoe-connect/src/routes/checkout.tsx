      ...
      <SiteLayout>
        <div className="mx-auto max-w-3xl px-4 py-24 text-center">
          ...
          <p className="mt-4 text-muted-foreground">
            You need to add products to your inquiry basket before checking out.
          </p>
          <Link
            to="/products"
            search={{ category: undefined, gender: undefined }}
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-primary px-8 py-4 font-bold text-white transition-all hover:bg-ink"
          >
            Browse Catalog
          </Link>
        </div>
            ...
            {step === "success" ? (
              ...
              >
                ...
                <p className="text-lg text-muted-foreground mb-8">
                  Your wholesale manufacturing order <strong className="text-ink">{orderId}</strong>{" "}
                  is confirmed. We have received your 30% advance payment.
                </p>
                <div className="rounded-2xl border border-border bg-white p-6 premium-shadow mb-8 text-left">
                  <h3 className="font-bold text-ink mb-4 border-b border-border pb-2">
                    Next Steps
                  </h3>
                  <ul className="space-y-4 text-sm text-muted-foreground">
                    <li className="flex gap-3">
                      <span className="font-bold text-primary">1.</span> Our production manager will
                      contact you within 24 hours to confirm sizing runs.
                    </li>
                    <li className="flex gap-3">
                      <span className="font-bold text-primary">2.</span> Production will begin
                      immediately. Expected lead time is 14-21 days.
                    </li>
                    <li className="flex gap-3">
                      <span className="font-bold text-primary">3.</span> You will be invoiced for
                      the remaining 70% balance prior to dispatch.
                    </li>
                  </ul>
                </div>
                <Link
                  to="/"
                  className="inline-flex items-center gap-2 rounded-full bg-ink px-8 py-4 font-bold text-white transition-all hover:bg-primary"
                >
                  Return to Home
                </Link>
              </motion.div>
              ...
              >
                ...
                <div className="lg:col-span-7 xl:col-span-8">
                  ...
                  <div className="mb-8 flex items-center gap-4 text-sm font-bold uppercase tracking-wider text-muted-foreground">
                    ...
                    >
                      <span
                        className={`grid h-6 w-6 place-items-center rounded-full text-[10px] ${step === "shipping" ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                      >
                        1
                      </span>
                      Shipping Details
                    ...
                    <div
                      className={`flex items-center gap-2 ${step === "payment" ? "text-primary" : ""}`}
                    >
                      <span
                        className={`grid h-6 w-6 place-items-center rounded-full text-[10px] ${step === "payment" ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                      >
                        2
                      </span>
                      Payment
                  ...
                  {step === "shipping" ? (
                    <div className="rounded-2xl border border-border bg-white p-6 md:p-8 premium-shadow">
                      ...
                      <form onSubmit={handleShippingSubmit} className="space-y-6">
                        <div className="grid gap-6 md:grid-cols-2">
                          <div>
                            ...
                            <input
                              required
                              type="text"
                              className="w-full rounded-xl border border-input bg-background px-4 py-3.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary"
                              placeholder="e.g. Metro Shoes"
                            />
                          </div>
                          <div>
                            <label className="mb-2 block text-xs font-bold text-ink">
                              NTN / GST Number (Optional)
                            </label>
                            <input
                              type="text"
                              className="w-full rounded-xl border border-input bg-background px-4 py-3.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary"
                        ...
                        <div>
                          <label className="mb-2 block text-xs font-bold text-ink">
                            Delivery Address (Godown / Shop)
                          </label>
                          <textarea
                            required
                            rows={3}
                            className="w-full rounded-xl border border-input bg-background px-4 py-3.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary"
                            placeholder="Complete address for truck delivery"
                          />
                        </div>
                        <div className="grid gap-6 md:grid-cols-2">
                          <div>
                        ...
                        <div className="pt-4">
                          <button
                            type="submit"
                            className="w-full rounded-xl bg-ink px-4 py-4 text-sm font-bold text-white transition-all hover:bg-primary shadow-lg flex justify-center items-center gap-2"
                          >
                    ...
                    <div className="max-h-[300px] overflow-y-auto pr-2 space-y-4 mb-6">
                      {items.map((item) => (
                        <div key={item.slug} className="flex gap-3 text-sm">
                          <img
                            src={item.image}
                            className="w-16 h-16 rounded-lg object-cover bg-cream border border-border"
                          />
                          <div className="flex-1">
                            <p className="font-bold line-clamp-1">{item.name}</p>
                            <p className="text-xs text-muted-foreground mt-0.5">
                              Qty: {item.requestedQty} pairs @ {formatPKR(item.price)}
                            </p>
                          </div>
                      ...
                      <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 mt-6">
                        <div className="flex justify-between items-center mb-2">
                          <span className="font-bold text-primary flex items-center gap-1">
                            <ShieldCheck className="w-4 h-4" /> Advance Payable Now (30%)
                          </span>
                          <span className="font-display text-2xl font-black text-primary">
                            {formatPKR(advancePayment)}
                          </span>
                        </div>
                        <p className="text-[10px] text-primary/70 leading-relaxed">
                          Remaining 70% balance ({formatPKR(total - advancePayment)}) will be
                          invoiced upon production completion, prior to dispatch.
                        </p>
                      </div>
                    </div>
                  </div>
