...
export function toTrackingDto(tracking: any): any {
  ...
  const eventsSource = tracking.events?.length ? tracking.events : timeline;
  return {
    orderNumber: tracking.orderNumber || tracking.orderRef,
    orderReference: tracking.orderNumber || tracking.orderRef,
    status: tracking.status || tracking.currentStatus || "Processing",
    ...
    totalPairs: tracking.totalPairs,
    totalCartons: tracking.totalCartons,
    totalAmount: tracking.totalAmount,
    estimatedDelivery: tracking.estimatedDelivery,
    timeline,
    events: eventsSource.map((e: any) => ({
      timestamp: e.timestamp?.toISOString?.() || e.timestamp,
      status: e.status || e.step,
