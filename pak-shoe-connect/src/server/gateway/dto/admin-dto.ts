...
export interface SystemHealthMetricsDto {
  ...
  circuitBreakers: Record<
    string,
    { state: string; consecutiveFailures: number; available: boolean }
  >;
...
export function toAdminProductDto(p: any): AdminProductDto {
  return {
    ...
    unitPrice:
      typeof p.unitPrice === "number" ? formatCurrency(p.unitPrice) : p.unitPrice || "PKR 2,450",
...
export function toAdminAuditLogDto(l: any): AdminAuditLogDto {
  return {
    id: l.id,
    timestamp:
      l.timestamp?.toISOString?.() || l.createdAt?.toISOString?.() || new Date().toISOString(),
