        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {items.length === 0 ? (
            ...
            <ul className="space-y-6">
              {items.map((item) => (
                <li key={item.slug} className="flex gap-4 border-b border-border pb-6 last:border-0 last:pb-0">
                  ...
                  <div className="flex flex-1 flex-col">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center rounded-md border border-border">
                        <button 
                          onClick={() => updateQty(item.slug, Math.max(item.moq || 10, item.requestedQty - (item.moq || 10)))}
        ...
        {items.length > 0 && (
          <div className="border-t border-border bg-muted/30 p-6">
            ...
            <p className="text-[10px] text-center text-muted-foreground mt-4">
              By submitting an inquiry, you are requesting pricing based on your volume. No payment is required now.
            </p>
          </div>
        )}
      </SheetContent>
