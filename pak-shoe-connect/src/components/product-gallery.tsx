...
import { Heart, Share2, ZoomIn, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Play, Maximize2 } from "lucide-react";
...
export function ProductGallery({ product }: ProductGalleryProps) {
  ...
  const [activeTab, setActiveTab] = useState<"video" | "photos">(product.video ? "video" : "photos");
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-row gap-3 h-[400px] sm:h-[480px]">
        ...
        <div className="w-[60px] sm:w-[70px] flex-shrink-0 flex flex-col relative h-full">
          <button ref={thumbPrevRef} className="z-10 bg-white/80 hover:bg-white text-gray-700 rounded-md p-1 shadow-sm transition disabled:opacity-30 absolute top-0 left-0 right-0 flex justify-center pb-1">
              ...
              {product.images.map((img, index) => (
                <SwiperSlide key={index} className="cursor-pointer !h-[60px] sm:!h-[70px]">
                  <div className="w-full h-full rounded-md border-2 border-transparent transition-all overflow-hidden bg-white hover:border-primary/50 [&.swiper-slide-thumb-active]:border-primary">
                    <img src={img} alt={`Thumbnail ${index + 1}`} className="w-full h-full object-cover p-0.5" />
                  </div>
                </SwiperSlide>
              ))}
            </Swiper>
          ...
          <button ref={thumbNextRef} className="z-10 bg-white/80 hover:bg-white text-gray-700 rounded-md p-1 shadow-sm transition disabled:opacity-30 absolute bottom-0 left-0 right-0 flex justify-center pt-1">
        ...
        <div className="flex-1 relative rounded-lg overflow-hidden border border-border bg-gray-50 h-full flex flex-col">
          {activeTab === "video" && product.video ? (
            ...
            <div className="w-full h-full relative">
              ...
              <button ref={prevRef} className="absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-white hover:bg-gray-50 text-gray-700 rounded-full p-2 shadow-md transition disabled:opacity-30">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button ref={nextRef} className="absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-white hover:bg-gray-50 text-gray-700 rounded-full p-2 shadow-md transition disabled:opacity-30">
      ...
      <div className="mt-2 bg-[#F2F7FD] rounded-lg p-3 flex items-center gap-3 border border-[#E3EEFA]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-600 mt-0.5">
            <span className="flex items-center gap-1"><img src="https://flagcdn.com/w20/pk.png" alt="Pakistan" className="w-3.5 h-2.5" /> Lahore, PK</span>
