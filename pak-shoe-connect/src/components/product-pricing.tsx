...
export function ProductPricing({ tiers, samplePrice }: ProductPricingProps) {
  return (
    <div className="flex flex-col mb-6 mt-4">
      <div className="mb-3">
         <span className="bg-[#E52F2F] text-white text-xs font-bold px-2 py-0.5 rounded-sm">Lower priced than similar</span>
      </div>
          ...
          return (
            <div key={i} className="flex flex-col">
              <div className="flex items-baseline">
                ...
                <span className="text-2xl font-bold text-gray-900">{new Intl.NumberFormat("en-US").format(tier.pricePerPair)}</span>
              </div>
      ...
      {samplePrice && (
        <div className="mt-6 flex items-center justify-between border-y border-gray-100 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-700">
            ...
            <span className="font-semibold text-gray-900">PKR {new Intl.NumberFormat("en-US").format(samplePrice)}</span>
          </div>
