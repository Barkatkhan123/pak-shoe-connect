...
describe("Admin Console & Marketplace Control Plane Security Suite", () => {
  ...
  it("should block non-admin users (BUYER) from accessing admin endpoints with 403 Forbidden", async () => {
    const res = await apiGateway("/api/v1/admin/overview", "GET", {}, {}, {
  ...
  it("should restrict OPERATOR role from deleting products", async () => {
    const res = await apiGateway("/api/v1/admin/products/prod-test-99", "DELETE", {}, {}, {
  ...
  it("should allow ADMIN role to approve pending products", async () => {
    const res = await apiGateway("/api/v1/admin/products/prod-test-99/approve", "POST", {}, {}, {
      authorization: `Bearer ${adminToken}`,
    });
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  ...
  it("should enforce Four-Eyes approval for financial payout overrides", async () => {
    ...
    const initRes = await apiGateway("/api/v1/admin/finance/four-eyes/initiate", "POST", {
      amount: 1500000,
    }, {}, {
      authorization: `Bearer ${adminToken}`,
    });
    ...
    const adminApproveRes = await apiGateway("/api/v1/admin/finance/four-eyes/approve", "POST", {
    ...
    const superApproveRes = await apiGateway("/api/v1/admin/finance/four-eyes/approve", "POST", {
      requestId: initRes.body.data.requestId,
    }, {}, {
  ...
  it("should return system health telemetry including circuit breaker status", async () => {
    const res = await apiGateway("/api/v1/admin/system/health", "GET", {}, {}, {
      authorization: `Bearer ${adminToken}`,
    });
    expect(res.status).toBe(200);
