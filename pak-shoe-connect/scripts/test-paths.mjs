...
function testPath(pathStr) {
  return new Promise((resolve) => {
    ...
    const req = https.get(urlStr, { timeout: 8000, headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" } }, (res) => {
      let data = "";
    ...
    req.on("timeout", () => { req.destroy(); resolve({ path: pathStr, error: "Timeout" }); });
  });
}
